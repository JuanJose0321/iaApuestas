...
- **API key real filtrada en git** | `.env.example:17` | El archivo `.env.example` (rastreado en git desde el commit `a968ea0`, no es un placeholder) contenía una key de Groq real y funcional (prefijo `gsk_...`, redactada aquí a propósito) | **Revocar la key en console.groq.com de inmediato**, reemplazar la línea por `GROQ_API_KEY=` vacío, purgar del historial de git (BFG/filter-repo) y añadir un pre-commit hook de secret scanning (gitleaks/truffleHog). — **Estado: corregido en Fase 2** (placeholder en `.env.example`, historial purgado, hook de gitleaks añadido).
...
- **Servidor Flask de desarrollo como único punto de entrada** | `app.py:418` (`app.run(...)`) | No hay WSGI de producción (gunicorn/waitress), ni `SECRET_KEY`, ni límites de tamaño de request. El propio Flask lo advierte como "no apto para producción". | Servir con `waitress`/`gunicorn` detrás de un proceso supervisado; definir `SECRET_KEY` desde entorno.
...
- **Tests duplicados/dispersos** | raíz (`test_api_teams.py`, `test_app_imports.py`, `test_frontend_debug.py`, `test_tennis_integration.py`) + `tests/` | Dos convenciones de ubicación coexisten sin razón aparente; complica saber qué se ejecuta con `pytest` por defecto. | Consolidar todo bajo `tests/`.
...
- **`except Exception` amplio y silencioso** | `app.py:193`, `app.py:225-226`, `app.py:240-241` | Fallos de contexto de API, coherencia y narrativa LLM se tragan sin registrar el stacktrace (solo `debug` o nada), lo que puede esconder degradación sistemática (ej. la API externa lleva días caída) detrás de un "sin datos" silencioso. | Loguear a nivel `warning`/`error` con el stacktrace incluso cuando el fallback continúa.
...
- **Logs versionados en git** | `api_calls.log`, `logs/betbrain.log` | Archivos de log no deberían commitearse; crecen indefinidamente en el historial. | Añadir a `.gitignore`, eliminar del tracking.
